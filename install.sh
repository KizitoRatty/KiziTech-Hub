#!/bin/bash
# KiziTech Hub UDP Installer – Debian/Ubuntu with VoIP Support
set -e
...
# ==================== NEW ATTRACTIVE HEADER (KiziTech) ====================
clear
echo -e "${W}╔══════════════════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${W}║${NC}                                                                              ${W}║${NC}"
...
echo -e "${W}║${NC}                                                                              ${W}║${NC}"
echo -e "${W}╠══════════════════════════════════════════════════════════════════════════════╣${NC}"
echo -e "${W}║${NC}                         ${G}${BOLD}KiziTech Hub UDP Tunnel Installer${NC}                       ${W}║${NC}"
echo -e "${W}╚══════════════════════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${G}  OS       :${NC} $OS"
...
echo -e "${G}  IP       :${NC} $IP"
echo -e "${G}  ISP      :${NC} $ISP"
echo -e "${G}  Admin    :${NC} @KizitoRatty"
echo ""

# ==================== REST OF THE INSTALLER (unchanged core) ====================
...
openssl req -new -newkey rsa:4096 -days 365 -nodes -x509 \
    -subj "/C=GH/ST=Accra/L=Accra/O=KiziTech/CN=kizitech" \
...
echo -e "${Y}[+] Installing KiziTech Hub panel (Ratty) from your repo...${NC}"
# Replace the URL below with the actual location of your Ratty panel script
PANEL_URL="https://raw.githubusercontent.com/OfficialOnePesewa/OFFICIAL-ONEPESEWA-UDP/main/onepesewa"
wget -qO /usr/local/bin/Ratty "$PANEL_URL" || {
    echo -e "${R}Failed to download panel from $PANEL_URL${NC}"; exit 1
}
chmod +x /usr/local/bin/Ratty
...
echo -e "${C}====================================================${NC}"
echo -e "${Y} Type 'Ratty' to open the KiziTech Hub panel.${NC}"
echo -e "${C}====================================================${NC}"
...
echo ""
echo -e "${G}All done! Type 'Ratty' to manage your users.${NC}"
